...
def transform_products() -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(f"Failed to transform bronze products: {e}", exc_info=True)
        raise 
...
def transform_users() -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(f"Failed to transform bronze users: {e}", exc_info=True)
        raise 
...
def transform_orders() -> pd.DataFrame:
    ...
    try: 
        ...
        if len(invalid) > 0:
            # TODO (TP3): Replace with logger.warning(...)
            logger.info(f"    ⚠️  {len(invalid)} orders with invalid status (removed)")
        ...
        logger.error(f"Failed to transform bronze orders: {e}", exc_info=True)
        raise 
...
def transform_order_line_items() -> pd.DataFrame:
    ...
    try:
        ...
        if len(invalid_qty) > 0:
            # TODO (TP3): Replace with logger.warning(...)
            logger.info(f"    ⚠️  {len(invalid_qty)} rows with quantity <= 0 (removed)")
        df = df[df["quantity"] > 0]
        ...
        logger.error(f"Failed to transform bronze order line items: {e}", exc_info=True)
        raise
